...
def create_graph(pose_length, correlations, start, end):
    ...
    for pose_index in range(pose_length):
        ...
        for frame_index in range(1, frames):
            ...
            node1 = pose1 + "," + frame1
            # print("node1: ", node1)
            pose2 = str(f"{(pose_index + 3):03d}")
            frame2 = str(f"{(frame_index + 1):03d}")
            if pose_index == (pose_length - 2):
                pose2 = str(f"{1:03d}")
            if pose_index == (pose_length - 1):
                pose2 = str(f"{2:03d}")
            node2 = pose2 + "," + frame2
            # print("node2: ", node2)
            if pose_index < (pose_length - 2):
                edges.append((node1, node2, correlations[pose_index + 2, frame_index]))
                # print("Weight: ", correlations[pose_index + 2, frame_index])
            else:
                if pose_index == (pose_length - 2):
                    edges.append((node1, node2, correlations[0, frame_index]))
                    # print("Weight: ", correlations[0, frame_index])
                if pose_index == (pose_length - 1):
                    edges.append((node1, node2, correlations[1, frame_index]))
                    # print("Weight: ", correlations[1, frame_index])
...
# Path settings
pose_path = r'G:\CASIA_B\Selected_Half_Cycles\Frames_14\Averaged_Frames'
path1 = r"G:\CASIA_B\GaitDatasetB-silh_090_Normalized_Alinged"
save_path = r"G:\CASIA_B\GaitDatasetB-silh_090_Normalized_Alinged_DGEI14"
